...
with col1:
    st.markdown(f'<p style="color:#6b7280;font-size:0.85rem;margin:0.5rem 0 0 0;">👤 {st.session_state.user_email}</p>', unsafe_allow_html=True)
with col2:
    ...
    if st.session_state.pedidos:
        ...
        for idx, p in enumerate(st.session_state.pedidos):
            ...
            if st.session_state.editando_idx == idx:
                with st.container():
                    st.markdown(
                        f'<div style="background:rgba(124,58,237,0.2);padding:0.5rem 0.75rem;border-radius:8px;margin-bottom:0.5rem;border-left:3px solid #a78bfa;">'
                        f'<small style="color:#c4b5fd;font-weight:600;">Editando corte #{idx+1}</small>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                ...
                # Vista normal: corte con botones de acción más limpios
                col_info, col_edit, col_del = st.columns([3, 1.3, 1.3])
                with col_info:
                    st.markdown(
                        f'<div style="padding:0.55rem 0.25rem 0.55rem 0;color:white;">'
                        f'<span style="font-weight:600;color:white;font-size:1rem;">{p.largo}m</span>'
                        f'<span style="color:rgba(255,255,255,0.5);margin-left:0.5rem;font-size:0.9rem;">× {p.cantidad}</span>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                with col_edit:
                    if st.button("Editar", key=f"edit_btn_{idx}", use_container_width=True):
                        ...
                        st.rerun()
                with col_del:
                    if st.button("Quitar", key=f"del_btn_{idx}", use_container_width=True):
